...
function theme756_preprocess_search_result(&$vars) {
    ...
    if ($node->nid && $node->type == 'article') {
        ...
        pss_prepare_vars_for_search($node, $vars);
    }
}
...
function theme756_preprocess_html(&$vars) {
    ...
    if (!path_is_admin(current_path())) {
        $vars['classes_array'][] = 'not-admin-page';
    } else {
    ...
    $arg0 = arg(0);
    $arg1 = arg(1);
    if (isset($arg0, $arg1) && $arg0 == 'node' && is_numeric($arg1)) {
        $node = node_load($arg1);
        $node = isset($node) ? $node : NULL;
        if (isset($node) && isset($node->field_article_type)) {
            $type = reset($node->field_article_type);
            $articleType = isset($type, $type[0], $type[0]['tid']) ? 'page-taxonomy-type-' . $type[0]['tid'] : 'page-taxonomy-type-';
            $vars['classes_array'][] = $articleType;
        }
    }
...
function theme756_preprocess_node(&$vars) {
    ...
    if($vars['type'] == 'article' && $vars['view_mode'] == 'teaser') {
        ...
        pss_prepare_vars_for_search($vars['node'], $vars);
    }
}

function pss_prepare_vars_for_search($node, &$vars) {
    $lang_lat = 'lat';
